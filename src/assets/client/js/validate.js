...
//các ký tự a-z A-Z 0-9 và gạch dưới
Validator.isChar = function (selector, message) {
    return {
        selector: selector,
        test: function (value) {
            var regex = /^[A-Za-z0-9_.]+$/;
            return regex.test(value) ? undefined : message || 'Mã đăng nhập chứa ký tự không phù hợp';
        }
    };
}

//full name(việt nam)
Validator.fullName = function (selector, message) {
    return {
        selector: selector,
        test: function (value) {
            ...
            return regex.test(value) ? undefined : message || 'Mục bạn nhập không đúng dịnh dạng';
        }
    };
}
